...
public class JwtService {
     ...
     * @throws AppException.TokenException nếu token không hợp lệ
     */
    public boolean validateAccessToken(String token) {
        if (!jwtUtil.validateTokenFormat(token)) {
            throw new AppException.TokenException("Token không hợp lệ hoặc đã hết hạn");
        }
        return true;
    }
     ...
     * @return true nếu hợp lệ
     * @throws AppException.TokenException nếu token không hợp lệ hoặc không khớp
     *                                     email
     */
    public boolean validateAccessToken(String token, String email) {
        ...
        if (!email.equals(tokenEmail)) {
            throw new AppException.TokenException("Token không khớp với email");
        }
     ...
     * @param token JWT token
     * @return User ID
     * @throws AppException.TokenException nếu không thể extract
     */
    public Long getUserIdFromToken(String token) {
        try {
            ...
            if (userId == null) {
                throw new AppException.TokenException("Token không chứa userId");
            }
            return userId;
        } catch (Exception e) {
            throw new AppException.TokenException("Không thể extract userId từ token: " + e.getMessage());
        }
    }
    ...
    public String getEmailFromToken(String token) {
        try {
            ...
            throw new AppException.TokenException("Không thể extract email từ token: " + e.getMessage());
        }
    }
    ...
    public String getScopeFromToken(String token) {
        try {
            ...
            throw new AppException.TokenException("Không thể extract scope từ token: " + e.getMessage());
        }
    }
    ...
    public Long getRoleIdFromToken(String token) {
        try {
            ...
            throw new AppException.TokenException("Không thể extract roleId từ token: " + e.getMessage());
        }
    }
    ...
    public Long getSchoolIdFromToken(String token) {
        try {
            ...
            throw new AppException.TokenException("Không thể extract schoolId từ token: " + e.getMessage());
        }
    }
